...
AsyncUDPPacket::AsyncUDPPacket(AsyncUDP* udp, ip4_addr_t* localIp, uint16_t localPort, ip4_addr_t* remoteIp, uint16_t remotePort, uint8_t* data, size_t len)
{
    _udp = udp;
    _localIp = localIp;
...
void AsyncUDP::_recv(udp_pcb* upcb, pbuf* pb, ip4_addr_t* addr, uint16_t port)
{
...
void AsyncUDP::_s_recv(void* arg, udp_pcb* upcb, pbuf* p, ip4_addr_t* addr, uint16_t port)
#else
void AsyncUDP::_s_recv(void* arg, udp_pcb* upcb, pbuf* p, const ip4_addr_t* addr, uint16_t port)
#endif
{
    reinterpret_cast<AsyncUDP*>(arg)->_recv(upcb, p, (ip4_addr_t*)addr, port);
}

bool AsyncUDP::listen(ip4_addr_t* addr, uint16_t port)
{
    close();
    _pcb = udp_new();
    if (_pcb == NULL) {
...
bool AsyncUDP::listenMulticast(ip4_addr_t* addr, uint16_t port, uint8_t ttl)
{
    close();
    if (!ip_addr_ismulticast(addr)) {
        return false;
    }
    ip4_addr_t multicast_if_addr;
...
bool AsyncUDP::connect(ip4_addr_t* addr, uint16_t port)
{
    close();
    _pcb = udp_new();
    if (_pcb == NULL) {
...
size_t AsyncUDP::writeTo(const uint8_t* data, size_t len, ip4_addr_t* addr, uint16_t port)
{
...
size_t AsyncUDP::sendTo(AsyncUDPMessage& message, ip4_addr_t* addr, uint16_t port)
{
    if (!message) {
        return 0;
    }
