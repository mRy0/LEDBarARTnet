...
protected:
    ...
    ip4_addr_t* _localIp;
    uint16_t _localPort;
    ip4_addr_t* _remoteIp;
    uint16_t _remotePort;
    uint8_t* _data;
    size_t _len;
public:
    AsyncUDPPacket(AsyncUDP* udp, ip4_addr_t* localIp, uint16_t localPort, ip4_addr_t* remoteIp, uint16_t remotePort, uint8_t* data, size_t len);
...
protected:
    ...
    void _recv(udp_pcb* upcb, pbuf* pb, ip4_addr_t* addr, uint16_t port);
#if LWIP_VERSION_MAJOR == 1
    static void _s_recv(void* arg, udp_pcb* upcb, pbuf* p, ip4_addr_t* addr, uint16_t port);
#else
...
public:
    ...
    bool listenMulticast(ip4_addr_t* addr, uint16_t port, uint8_t ttl = 1);
    ...
    size_t writeTo(const uint8_t* data, size_t len, ip4_addr_t* addr, uint16_t port);
    ...
    size_t sendTo(AsyncUDPMessage& message, ip4_addr_t* addr, uint16_t port);
